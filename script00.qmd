...
df_2024 <- read_csv("fortune1000_2023.csv")|>
  mutate(Year="2023")%>%
  rename(MarketCap_March=MarketCap_March31_M)

df_tot<- rbind(df1,df_2024) #le fichier superpose l'ensemble des observations

```

Pour garder les mêmes entreprises, et avoir leur données 2024 et 2023, on réalise une jointure à gauche (les données de 2024 sont mises à jour avec les données 2023)
...
summary(fit)
```
Il est temps d'introduire d'autre variables.
